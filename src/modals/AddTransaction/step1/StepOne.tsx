  ...
  <div className="flex flex-col gap-8">
    <div className="flex flex-col gap-4">
      ...
      <div className="relative">
        <span className="absolute left-6 top-1/2 -translate-y-1/2 text-4xl font-light text-[var(--text-muted)]">$</span>
        <input
          type="number"
          inputMode="decimal"
          placeholder="0.00"
    ...
    <div className="grid grid-cols-2 gap-4 sm:gap-6">
      {(['income', 'expense'] as const).map(option => (
        <button
          ...
          className={`group relative flex flex-col items-center gap-3 overflow-hidden rounded-[2rem] border border-[var(--border-glass)] bg-[var(--surface-glass)] p-6 transition backdrop-blur-md ${
            option === 'income'
              ? 'hover:border-income/40 hover:bg-income/5'
              : 'hover:border-expense/40 hover:bg-expense/5'
          }`}
        >
          <div className={`flex h-12 w-12 items-center justify-center rounded-2xl transition group-hover:scale-110 ${
            option === 'income'
